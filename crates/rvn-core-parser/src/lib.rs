...
use std::path::Path;
use crate::types::{MetadataValueType, GGMLType, Value};
...
impl Default for ParsedGGUF {
    fn default() -> Self {
        ParsedGGUF {
            header: Header::default(),      // tensor_count = 0, metadata_kv_count = 0
            metadata: HashMap::new(),       // no metadata yet
            tensors: HashMap::new(),        // no tensors yet
        }
    }
}
...
pub fn parse_tensors_common(cursor: &mut Cursor<&[u8]>, tensor_count: u64) -> Result<HashMap<String, Tensor>> {
    ...
    for _ in 0..tensor_count {
        ...
        let tensor = Tensor { name: name.clone(), kind, size, shape };
